...
| $G_t\in\mathbb{R}^{M_a\times d}$ | $G_t=\mathrm{AnchorPool}(C_t)$ | Always-visible global anchor tokens | B4 (default), <span style="color:#BDBDBD">B7 (optional ablation)</span> | Yes |
...
| $l_q^{\text{ring}}$ | Ring-pooled descriptor from LocalSample | Structured local context by distance bands | B3, B5, B7, B8 | Yes (through local heads) |
| $m_q$ | $m_q=\sigma(w_m^\top[f_{q,0};l_q^{\text{ring}}]+b_m)$ | Local reliability score for center-surround evidence | B4, B5, B8 | Yes |
...
| $z_q$ | $z_q=W_z[f_{q,0};l_q^{\text{ctx}};l_q^{\text{ring}};e(q)]$ | Query token (point+local+position) | B4, B5, B8 | Yes |
| $h_{\text{point},q}$ | Point-locked branch from $f_{q,0}$ and $e(q)$ | Target-pixel identity representation | B3, B7, B8, B9 | Yes |
| $h_{\text{ctr},q}^{0}$ | Early center token from $(h_{\text{point},q},l_q^{\text{ctx}},l_q^{\text{ring}})$ | Center token before routing/sampling | B4, B5, B7 | Yes |
...
| $h_{\text{ctx}}$ | Context branch output after reciprocal center-context coupling | Context correction representation | B8 | Yes |
...
\left(l_j^{\text{ctx}},l_j^{\text{ring}},m_j\right)=\mathrm{LocalSample}(F_t^{(1)},M_t,q_j)
$$
...
- $l_j^{\text{ring}}$ is distance-structured local context (near to far in local window).
- $m_j$ is local reliability used to coordinate local/global budget downstream.

Used next:

- B3 query token and center branch, B4 routing confidence, B5 conditioning, B8 fusion.

Trainable:

- Sampling op: No.
- Local heads (`MLP_local`, `MLP_ring`, reliability head): Yes.

#### B1.1) LocalSample helper function (explicit runtime definition)

Use a coordinated local strategy: fixed dense core + learned offsets + ring pooling. This keeps center-surround evidence stable while adapting to directional structure.
...
$$
Define ring partition on local offsets:
$$
\Omega_{j,b}=\{\delta\in\Omega_j\setminus\{(0,0)\}\mid \rho_{b-1}<\|\delta\|_{\infty}\le \rho_b\},\qquad
...
$$
$$
r_{j,b}=\frac{1}{|\Omega_{j,b}|}\sum_{\delta\in\Omega_{j,b}}f_{j,\delta},\qquad
l_j^{\text{ring}}=\operatorname{MLP}_{\text{ring}}\!\big([r_{j,1};\ldots;r_{j,B_r}]\big)
$$
$$
m_j=\sigma\!\left(w_m^\top\!\left[\mathrm{LN}(f_{j,0});\mathrm{LN}(l_j^{\text{ring}})\right]+b_m\right)
$$

Meaning:

- This guarantees one exact center feature plus dense local evidence.
- Ring pooling gives structured near-vs-far local context, not just one flattened local vector.
- $m_j$ quantifies whether this query can rely more on local evidence or should request stronger global support.

Used next:

- B3 (query token and center branch), B4 (routing confidence), B5 (sampling plan), and B8 (fusion).

Trainable:

- Learned local offset head $\{W_m^{\text{loc}},b_m^{\text{loc}}\}$: Yes.
- Bilinear sampling op and set union: No.
- $\operatorname{MLP}_{\text{local}}$, $\operatorname{MLP}_{\text{ring}}$, and reliability head $(w_m,b_m)$: Yes.
...
\tilde l_j^{\text{ring}}=\mathrm{LN}(l_j^{\text{ring}}),\qquad
\tilde e_j=\mathrm{LN}(e_j)
$$
$$
z_j=W_z[\tilde f_{j,0};\tilde l_j^{\text{ctx}};\tilde l_j^{\text{ring}};\tilde e_j]+b_z,\qquad
...
$$
$$
u_{\text{ctr},j}=\mathrm{GELU}\!\left(W_{c1}[h_{\text{point},j};\tilde l_j^{\text{ctx}}]+b_{c1}\right)\odot \mathrm{GELU}\!\left(W_{c2}\tilde l_j^{\text{ring}}+b_{c2}\right)
$$
$$
...
- $h_{\text{ctr},j}^{0}$ is an early center token conditioned on surrounding local evidence with explicit multiplicative nonlinearity.

Intuition (why this step exists):

- A dedicated point branch prevents the exact target signal from being diluted by surrounding/global features.
- The routing token now sees center + near context + ring context, so global retrieval is coordinated with local structure.
...
- Multiplicative interaction forces non-linear coupling between exact center identity and surrounding structure.
...
- Yes ($W_z$, $W_{p1}$, $W_{p2}$, $W_{\text{skip}}$, $W_{c1}$, $W_{c2}$, $W_{c3}$, and norm layers).

#### B4) Coarse routing with connectivity-safe token subset

Build center-aware routing query:
$$
q_j^{\text{route}}=\mathrm{LN}\!\left(W_r[z_j;h_{\text{ctr},j}^{0};m_j]+b_r\right)
$$
Compute content routing scores over coarse bank:
$$
\tilde\alpha_j=\mathrm{softmax}\!\left(\frac{(W_q q_j^{\text{route}})(W_k C_t)^\top}{\sqrt d\,\tau_r}\right)
$$
Training-time differentiable sparse selection:
$$
R_j^{\text{train}}=\operatorname{TopR}_{\text{ST}}(\tilde\alpha_j)
$$
Inference-time hard selection:
$$
R_j^{\text{infer}}=\operatorname{TopR}(\tilde\alpha_j)
$$
Use
$$
R_j=
...
i_j^{\text{loc}}=\arg\min_{i\in\{1,\ldots,P_c\}}\left\|\mathbf{p}_i-q_j^{(2)}\right\|_2^2
$$
...
- $l_j^{\text{ring}}$ and $m_j$: structured local context and local reliability from B1.1.
...
$$
\Delta \mathbf{p}_{j,r}^{\text{meta}}=\mathbf{p}_r^{(\ell_r)}-\mathbf{p}_{q_j}^{(\ell_r)}
$$
where $\tau_r$ is the token-type embedding (`routed`, `coverage`, or `local-anchor`).
$$
\hat u_{j,r}=[z_j;\ h_{\text{point},j};\ h_{\text{ctr},j}^{0};\ f_{j,0};\ l_j^{\text{ctx}};\ l_j^{\text{ring}};\ m_j;\ \bar c_j;\ \psi(\Delta \mathbf{p}_{j,r}^{\text{meta}},\ell_r,\tau_r)]
$$
$$
...
$$
\tilde\beta_{r,h,\ell,m}=\beta_{r,h,\ell,m}+\mathbf{1}[r=i_j^{\text{loc}}]\cdot b_{\text{loc}}+\kappa_m\,m_j
$$
where $b_{\text{loc}}$ and $\kappa_m$ are learnable scalar parameters.
...
$$
$$
a_{r,h,\ell,m}=\frac{\exp(\tilde\beta_{r,h,\ell,m})}{\sum_{(r',h',\ell',m')\in\mathcal{D}_j}\exp(\tilde\beta_{r',h',\ell',m'})}
$$
$$
...
- Because $u_{j,r}$ contains center, near-surrounding, reliability, and global branches together, sampling policy is coordinated instead of separated.
...
$$
$$
p_{\text{norm},r,h,\ell,m}=\mathrm{NormalizeToGrid}(p_{\text{raw},r,h,\ell,m};H_\ell,W_\ell)
$$
$$
f_{r,h,\ell,m}=\operatorname{GridSample}_{\text{reflect}}\!\left(F_t^{(\ell)},\ p_{\text{norm},r,h,\ell,m}\right)
$$
$$
...
$$
\mathcal{D}_j^{\text{near}}=\left\{(r,h,\ell,m)\in\mathcal{D}_j\ \middle|\ r=i_j^{\text{loc}}\ \text{or}\ \left\|p_r^{(\ell)}-p_{q_j}^{(\ell)}\right\|_{\infty}\le r_{\text{near}}\right\}
$$
with recommended $r_{\text{near}}=2$ (in level-$\ell$ feature coordinates).
$$
h_{\text{near},j}^{\text{raw}}=\sum_{(r,h,\ell,m)\in\mathcal{D}_j^{\text{near}}}a_{r,h,\ell,m}f_{r,h,\ell,m},\qquad
h_{\text{def},j}=\sum_{(r,h,\ell,m)\in\mathcal{D}_j}a_{r,h,\ell,m}f_{r,h,\ell,m}
$$
...
$$
$$
\tilde p\leftarrow\mathrm{NormalizeToGrid}(x_{r,h,\ell,m},y_{r,h,\ell,m}),\qquad
f_{r,h,\ell,m}=\operatorname{GridSample}_{\text{reflect}}(F_t^{(\ell)},\tilde p)
$$
...
$$
h_{\text{near},j}=\mathrm{LN}\!\left(W_{\text{near}}\!\left[h_{\text{near},j}^{\text{raw}};l_j^{\text{ctx}};l_j^{\text{ring}}\right]+b_{\text{near}}\right)
$$
$$
...
$$
h_{\text{ctr},j}^{+}=\mathrm{LN}\!\left(h_{\text{ctr},j}+\mathrm{MHA}\!\left(Q=W_q^{c}h_{\text{ctr},j},K=W_k^{c}\tilde T_j,V=W_v^{c}\tilde T_j\right)\right)
$$
Reciprocal context update (center writes back to selected context):
$$
\eta_{j,n}=\sigma\!\left(w_{\eta}^{\top}[t_{j,n};h_{\text{ctr},j}^{+};z_j]+b_{\eta}\right),\qquad
t_{j,n}^{+}=\mathrm{LN}\!\left(t_{j,n}+\eta_{j,n}W_{\text{up}}h_{\text{ctr},j}^{+}\right),\quad n\in I_j
$$
...
$$
$$
h_{\text{ctx},j}=\mathrm{LN}\!\left(\mathrm{MLP}_{\text{ctx}}\!\left([h_{\text{ctx},j}^{0};h_{\text{near},j};h_{\text{global},j};m_j]\right)\right)
$$
...
- Two-way coupling lets center and context update each other before final depth decoding.
...
2. M2 `QueryEncode` (merge B1+B2+B3):
   $$
   q_j\rightarrow (f_{j,0},l_j^{\text{ctx}})\ (\text{hybrid local }N_{\text{loc}}\approx121)\rightarrow e_j\rightarrow (z_j,h_{\text{point},j})
   $$
3. M3 `SparsePlan` (merge B4+B5):
   $$
   (z_j,h_{\text{ctr},j}^{0},f_{j,0},l_j^{\text{ctx}},l_j^{\text{ring}},m_j,h_{\text{point},j},C_t,G_t)\rightarrow (S_j,\bar c_j,\Delta p,a)
   $$
4. M4 `SparseReadAggregate` (merge B6+B7):
   $$
   (F_t^{(1:3)},S_j,\Delta p,a,\bar c_j,h_{\text{point},j},h_{\text{ctr},j}^{0},l_j^{\text{ctx}},l_j^{\text{ring}})\rightarrow (h_{\text{near},j},h_{\text{global},j},h_{\text{ctr},j})
   $$
5. M5 `FuseDecode` (merge B8+B9, <span style="color:#BDBDBD">optional B10 uncertainty</span>):
   $$
   (h_{\text{near},j},h_{\text{global},j},h_{\text{ctr},j},z_j,m_j,\bar c_j,H_t,s_t,b_t)\rightarrow (\hat d_j,\sigma_j)
   $$

Default deployment profile:

- Keep hybrid local sampler at default budget (`N_loc\approx121`) without hard-query expansion.
- Use a single-pass coupled B8 block (center, near-surround, and global in one constrained fusion; no deep iterative loop).
- Disable the extra B7 anchor-attention branch (anchors are already in $\bar c_j$).
...
2. `K1_QueryEncode`: merge B1-B3, $q\rightarrow f_{q,0},l_q^{\text{ctx}},l_q^{\text{ring}},m_q,e(q),z_q,h_{\text{point},q},h_{\text{ctr},q}^{0}$ (hybrid `LocalSample`, default $N_{\text{loc}}\approx121$).
3. `K2_SparsePlan`: merge B4-B5, $(z_q,h_{\text{ctr},q}^{0},f_{q,0},l_q^{\text{ctx}},l_q^{\text{ring}},m_q,h_{\text{point},q},C_t,G_t)\rightarrow (S_q,\bar c_q,\Delta p,a)$.
...
- <span style="color:#BDBDBD">add adaptive local expansion in `K1`,</span>
